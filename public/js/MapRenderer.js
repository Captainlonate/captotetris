...
/* 
 * redrawOffScreen calls drawTileAt
 * ctx - the context to use to draw on the desired canvas
 * tileId - what "kind" of tile to draw (snow dirt grass)
 * cellX - which column the cell is in
 * cellY - which row the cell is in
 */
_p._drawTileAt = function(ctx, tileId, cellX, cellY) {

    // Position of the tile inside of a tile sheet
    // tile 1 is actually cell 0 and tileId 0
    // tiles per row is 5. There is actually a tile 4 in theory, but don't use it
    // The number of cells over resetting to 0 after 5 is reached
    var srcX = (tileId%this._tilesPerRow)*this._tileSize;
    // The number of rows down * the size of each row
...
_p.updateMapData = function(mapData) {
	this._mapData = mapData;
	this._offDirty = true;
};

_p.getXandY = function() {
	return {
		x: this._x,
		y: this._y,
